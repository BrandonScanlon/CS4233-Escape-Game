      ...
      case ORTHOGONAL:
        ...
        while(deltaRow != 0 || deltaCol != 0) {
          ...
          CoordinateImpl leftCoord = tempGameBoard.getBoardLocation(newFrom.getRow(), newFrom.getColumn() - 1);
            ...
            /** Right **/
            if(((deltaRow <= 0 && deltaCol > 0) || (deltaRow < deltaCol && deltaCol > 0)) && ((Math.abs(deltaRow) <= Math.abs(deltaCol))) && !checkForObstacles(tempGameBoard, rightCoord)) {
              moveRight();
              moves++;
              continue;
            }
...
/******************************************************** LINEAR *************************************************************************/
      case LINEAR:
        ...
        while(deltaRow != 0 || deltaCol != 0) {
          ...
          if(((deltaRow == deltaCol - 2 && deltaRow != 0 && deltaCol != 0) || (Math.abs(deltaRow) == Math.abs(deltaCol) && deltaRow < 0)) && (direction == "NONE" || direction == "DOWNRIGHT")) { // Move down right
            if(tempGameBoard.getBoardLocation(newFrom.getRow(), newFrom.getColumn()).getPieceName() == EscapePiece.PieceName.BIRD) {
              moveDownRight();
            } else {
              if(tempGameBoard.getBoardLocation(newFrom.getRow() + 1, newFrom.getColumn()).getLocationType() != LocationType.CLEAR) {
          ...
          } else if(((deltaRow == deltaCol + 2 && deltaRow != 0 && deltaCol != 0) || (Math.abs(deltaRow) == Math.abs(deltaCol) && deltaCol < 0 && deltaRow > 0)) && (direction == "NONE" || direction == "UPLEFT")) { // Move up left
            if(tempGameBoard.getBoardLocation(newFrom.getRow(), newFrom.getColumn()).getPieceName() == EscapePiece.PieceName.BIRD) {
              moveUpLeft();
            } else {
              if(tempGameBoard.getBoardLocation(newFrom.getRow() + 1, newFrom.getColumn()).getLocationType() != LocationType.CLEAR) {
